...
| ARB-014 | events audit log writer + viewer API | 010 | — | Every state change in later tickets writes an event (verified by tests) | DONE (writer, reader and `GET /v1/events` built and tested; the "every state change" half is a standing obligation each later ticket carries, not something this one can close) | PENDING |
