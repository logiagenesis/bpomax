...
| ARB-031 | LLM package: provider config, JSON-schema validation, token/cost metering | 002 | B-08 | Invalid model output rejected and retried once; cost recorded per call | DONE (both clauses proven against a scripted transport, mutation-checked; prices transcribed from the published page with source and date; the Anthropic transport itself has never made a call — V-04, needs B-08) | PENDING |
