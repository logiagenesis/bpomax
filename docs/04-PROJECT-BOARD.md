...
| ID | Ticket | Depends | Blockers | Acceptance criteria | Status | SHA |
|---|---|---|---|---|---|---|
| ARB-010 | Supabase migrations: tenancy, marketplace, money, events, settings tables per 01-D | 004 | B-06, D-05 | `pnpm db:reset` applies cleanly; generated types committed | IN PROGRESS (claimed by session …tJv8 at 06:17 UTC 23/09/2026, on branch `claude/beautiful-tesla-b6goej`: `pnpm db:reset` and a second `pnpm db:seed` in CI's compose job, per docs/HANDOFF.md section 4) | 8c9c8fa |
...
| ARB-013 | Seed service_categories and seed market_price_bands flagged source='seed' | 010 | — | Seed idempotent; seed bands visibly labelled "seed" in UI | IN PROGRESS (claimed by session …tJv8 at 06:20 UTC 23/09/2026, on branch `claude/beautiful-tesla-b6goej`: `GET /v1/price-bands` and the "seed" label on the settings page, per docs/HANDOFF.md section 4. Already built: the 22-category taxonomy seeded idempotently, every band flagged source='seed', `pnpm db:seed`, and zero bands shipped (D-14).) | c21c34e |
