...
| ARB-030 | BullMQ setup, retries, dead-letter queue, worker health endpoint | 004 | B-07 | Failed job retried with backoff then lands in DLQ; health endpoint reports queues | DONE (proven against a real Redis, locally and as a CI service container; B-07's hosted instance is only needed to deploy, ARB-070) | SHA_030 |
