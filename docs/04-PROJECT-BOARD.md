...
| ARB-040 | Estimate worker (in-house → rate card → market band → AI-build) | 032, 013 | D-04, B-10 | Each estimate records its method; unit tests for each branch | IN PROGRESS (claimed 22/09/2026 18:40 SAST+0 by session …V4PPWs. The mechanism: every branch reads owner-supplied rows — in_house flags, rate cards, bands, an ai_build supplier's cards — and invents none; D-15 narrowed accordingly when it closes) | |
